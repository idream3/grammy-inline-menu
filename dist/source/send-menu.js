...
body, keyboard, context, other = {}) {
    ...
    if (isInvoiceBody(body)) {
        return context.replyWithInvoice(body.invoice.title, body.invoice.description, body.invoice.payload, body.invoice.currency, body.invoice.prices, createGenericOther(keyboard, other));
    }
    if (isTextBody(body)) {
...
telegram, menu, path) {
    return async (chatId, context, other = {}) => {
        ...
        if (isInvoiceBody(body)) {
            return telegram.sendInvoice(chatId, body.invoice.title, body.invoice.description, body.invoice.payload, body.invoice.currency, body.invoice.prices, createGenericOther(keyboard, other));
        }
        if (isTextBody(body)) {
